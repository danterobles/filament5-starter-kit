        ...
        return $table
            ...
            ->columns([
                TimelineEntry::make()
                    ->title('title')
                    ->content('description')
                    ->image(fn (Agenda $record): string => 'https://ui-avatars.com/api/?name='.urlencode($record->title)
                        .'&background='.ltrim($record->color ?? '3b82f6', '#')
                        .'&color=ffffff&size=80&bold=true&length=2'
                    )
                    ->author(
